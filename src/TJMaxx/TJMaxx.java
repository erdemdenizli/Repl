...
public class TJMaxx {
    ...
    public double getItemPrice(int catalogNumber)
    {
       for(int i=0; i<regularItems.size(); i++) {
         if(regularItems.get(i).getCatalogNumber() == catalogNumber) 
        	 return BigDecimal.valueOf(price*(100-discount)/100)
    ...
    {
        for(int i=0; i<onSaleItems.size(); i++) {
         if(onSaleItems.get(i).getName().equals(name)) return onSaleItems.get(i);
       }
    ...
    public void removeItem(int catalogNumber) {
        for(int i=0; i<onSaleItems.size(); i++) {
         if(onSaleItems.get(i).getCatalogNumber() == catalogNumber) onSaleItems.remove(onSaleItems.get(i));
       }
       
       for(int i=0; i<regularItems.size(); i++) {
         if(regularItems.get(i).getCatalogNumber() == catalogNumber) regularItems.remove(regularItems.get(i));
       }
    }
